...
func RegisterUserTools(s *server.MCPServer, handler UserHandler, cfg *config.Config) {
	...
	if !isReadOnly {
		...
		inviteUserTool := mcp.NewTool("invite_workspace_user",
			...
			// Directory clients require confirmation for access-changing tools.
			...
			mcp.WithString("role",
				mcp.Enum("member", "admin", "owner"),
				mcp.Description("Role to grant the user, applied when they accept the invitation. Defaults to member. You cannot grant a role above your own."),
			),
			mcp.WithString("workspace",
				mcp.Description("Optional workspace name to override the default workspace"),
			),
		...
		updateUserRoleTool := mcp.NewTool("update_workspace_user_role",
			...
			mcp.WithString("role",
				...
				mcp.Description("New role for the user. Ownership cannot be assigned this way."),
			),
			mcp.WithString("workspace",
				mcp.Description("Optional workspace name to override the default workspace"),
			),
