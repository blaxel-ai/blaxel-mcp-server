  ...
  server.tool(
    ...
    "Create a new Blaxel sandbox project locally using CLI",
    {
      directory: z
        ...
        .describe("Complete path to the directory to create for the sandbox (e.g., use `pwd`/my-sandbox for current directory)"),
      template: z
        .string()
        .optional()
        .describe(
    ...
    async ({ directory, template }) => {
      try {
        ...
        let command = `bl create-sandbox ${directory} -y${buildWorkspaceFlag(workspace)}`;
        if (template) {
